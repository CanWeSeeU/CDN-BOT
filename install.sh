...
# ── Constants ─────────────────────────────────────────────────────────────────
VERSION="1.0.1"
REPO_URL="https://github.com/CanWeSeeU/CDN-BOT.git"
...
# ── Help text shown before each credential prompt ────────────────────────────
...
fetch_files() {
    header "📂  Fetching application files"
    ...
    if [[ -d "${INSTALL_DIR}/.git" ]]; then
        ...
        # Remove non-git leftover if present
        rm -rf "${INSTALL_DIR}"
